"use client"; // Client side rendering so can listen for JS - button click and server not sending back just HTML
...
const FAQListItem = ({ qa }) => {
  ...
  return (
    <li key={qa.question}>
      ...
      {/* 2. Answer */}
      {/* {isOpen ? <div>{qa.answer}</div> : <></> // Js Turnary Operator} */}

      <div className={`${isOpen ? "block" : "hidden"} mt-3 mb-6 opacity-90`}>
        {/* Also uses Js Turnary Operator*/}
        {qa.answer}
      </div>
    </li>
  );
